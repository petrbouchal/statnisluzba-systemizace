...
syst_pocty_long <- lengthen_data(syst_all_fixed)
syst_pocty_long_0 <- lengthen_data(syst_all)
...
syst_tridy_0 <- syst_pocty_long_0 |>
  filter(date < "2022-04-01", kapitola_kod != "Celkem",
         !organizace_nazev %in% urady_nazvy_exclude) |>
  replace_na(list(pocet = 0)) |>
  mutate(trida = recode(trida, M = "0") |> as.numeric()) |>
...
syst_for_model_0 <- syst_platy_0 |>
  left_join(syst_tridy_0) |>
  mutate(trida_prumer = ((trida_predst * pocet_predst) +
           (trida_ostat * pocet_ostat))/pocet_celkem,
         trida_diff = trida_ostat - trida_predst,
         perc_predst = podil_predst * 100,
...
syst_for_model_2022 <- syst_for_model_0 |>
  filter(rok == 2022)

annual_model <- function(data) {
  lm(plat_prumer ~ perc_predst + trida_predst + trida_ostat + vztah + ustredni_organ,
     data = data)
}

model_by_year <- syst_for_model_0 |>
  group_by(rok) |>
  nest() |>
...
gg_resid_year <- ggplot(model_by_year |>
...
plat_model <- lm(plat_prumer ~ perc_predst + trida_predst + trida_ostat + vztah,
                 data = syst_for_model_2022)

summary(plat_model)
plat_model_tidy <- tidy(plat_model)
plat_model_aug <- augment(plat_model, syst_for_model_2022)
...
gg <- ggplot(plat_model_aug |>
       ...
       aes(plat_prumer, .fitted, tooltip = organizace_nazev)) +
  geom_abline() +
  geom_point_interactive(aes(colour = hygiena, shape = vztah, size = pocet_celkem), alpha = .6) +
  coord_fixed() +
...
gg_pts <- ggplot(plat_model_aug, aes(.resid, kapitola_typ, tooltip = organizace_nazev)) +
  ...
  geom_point_interactive(aes(colour = vztah), alpha = .7,
                         position = position_jitter(jitter.width = .05)) +
